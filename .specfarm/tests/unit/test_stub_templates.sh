...
if [[ -d "$PROJECT_ROOT/.specfarm/templates" ]]; then
    pass "templates directory exists"
else
    fail "templates directory not found at $PROJECT_ROOT/.specfarm/templates"
fi

# Test 3: Verify at least one template type exists (payroll)
if [[ -d "$PROJECT_ROOT/.specfarm/templates/payroll" ]]; then
...
TEMPLATE_DIR="$PROJECT_ROOT/.specfarm/templates/payroll"
